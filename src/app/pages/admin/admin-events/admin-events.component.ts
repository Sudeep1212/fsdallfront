...
export class AdminEventsComponent implements OnInit {
  ...
  onStartDateChange() {
    ...
    if (startDate) {
      ...
      this.eventForm.get('event_end_date')?.setValidators([Validators.required]);
  ...
  async confirmDelete() {
    ...
    try {
      ...
      await this.http
        .delete(`${this.apiUrl}/events/${this.selectedEventId}`, { responseType: 'text' })
        .toPromise();
      ...
      this.showErrorMessage(
        `Failed to delete event. Server might not be running or event doesn't exist.`
      );
    }
  }
